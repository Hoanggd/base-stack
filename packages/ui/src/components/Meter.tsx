...
import { Meter as AriaMeter, MeterProps as AriaMeterProps, composeRenderProps } from 'react-aria-components'
...
const Meter = ({ className, barClassName, fillClassName, children, ...props }: MeterProps) => (
    <AriaMeter className={composeRenderProps(className, className => cn('w-full', className))} {...props}>
...
function BsMeter({ label, className, showValue = true, ...props }: BsMeterProps) {
    return (
        <Meter
