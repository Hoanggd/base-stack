...
Instead of spreading authentication logic across folders such as `/pages`, `/widgets`, and `/features`, or debating which layer a file belongs to, nearly all authentication-related code is grouped together in an `/auth` module.
...
structure to your feature modules. The page.tsx files act as route handlers that orchestrate data fetching and component rendering, 
but they don't contain business logic or reusable components themselves.
...
- `config/` - Application-wide configuration files (API endpoints, feature flags)
...
- `hooks/` - Custom hooks that encapsulate this feature's stateful logic (usePosts, useCreatePost)
...
### 5. Import Flow and Best Practices
Understanding the import flow is crucial for maintaining a clean architecture over time.
<MdxImage src="/images/folder-import-flow.avif" alt="Folder import flow" />
**Dependency Rules:**
- **Routing depends on Features** - Route handlers import and compose feature components
...
- **Features NEVER depend on Routing** - Features shouldn't know about URLs or routing logic

## When to Break the Rules
Architecture is just a tool, and it's not perfect. You might break these rules when:
...
## Conclusion
A well-structured folder organization is the backbone of a scalable, maintainable codebase. By adopting a feature-based approach, you empower your team to work independently, reduce cognitive overhead, and make onboarding new developers much easier. Remember, the goal is to make your codebase intuitive, so that anyone can quickly find, understand, and modify the code they need.
